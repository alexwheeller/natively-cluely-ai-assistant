...
// Also include auditor electron files if they exist
const auditorDir = path.resolve(rootDir, 'auditor/electron');
if (fs.existsSync(auditorDir)) {
  entryPoints.push(...findTs(auditorDir).map(f => path.relative(rootDir, f)));
}
